...
def classify_keywords(text_lower):
    ...
    for kw in ALL_KEYWORDS:
        counts[kw] = text_lower.count(kw)  # counts occurrences
    # Flags for categories
    sustainability_hit = any(counts[k] > 0 for k in SUSTAINABILITY_KEYWORDS)
    entrepreneurship_hit = any(counts[k] > 0 for k in ENTREPRENEURSHIP_KEYWORDS)
    return counts, sustainability_hit, entrepreneurship_hit
...
def main(input_csv, output_detail_csv, output_summary_csv):
    ...
    for _, row in df_in.iterrows():
        ...
        if not isinstance(path, str) or not os.path.isfile(path):
            ...
            counts, sust_flag, entre_flag = classify_keywords(text_lower)
            hits, sust_flag, entre_flag = classify_keywords(text_lower)
            record = {
                "filepath": path,
                "exists": True,
                **{kw: counts[kw] for kw in ALL_KEYWORDS},
